...
void bis_receiver_start(uint8_t bis_index_to_sync_to)
{
	...
	while (true) {
		...
		/* Request the controller to use the single best subevent. This avoids
		 * accepting an arbitrary retransmission with a less deterministic
		 * receive path during precision measurements. */
		big_sync_param.mse = 1;
