...
void bis_transmitter_start(uint8_t retransmission_number, uint16_t max_transport_latency_ms)
{
	...
	struct bt_iso_big_create_param big_create_param = {
		/* Only one BIS is needed: all receivers sync to BIS index 1.
		 * The SoftDevice Controller reserves 2.5 ms for periodic
		 * advertising, so at the minimum 5 ms ISO interval a single BIS
		 * (NSE=2) is what fits - 8 BIS would exceed the BIG event budget.
		 */
		.num_bis = 1,
